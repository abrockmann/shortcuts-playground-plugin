...
- For **Format Date** with custom formats: set `WFDateFormatStyle=Custom` and put the pattern itself in `WFDateFormat` (e.g., `MMMM d, yyyy`, `yyyy-MM-dd`, `yyyy-MM-dd'T'HH:mm:ssXXXXX`). Do **not** emit `WFDateFormatString` and do not set `WFDateFormat=Custom`: with that shape the formatted date resolved empty on device (2026-09-20). Shortcuts itself writes the pattern into `WFDateFormat`, and the runtime ignores `WFDateFormatString` (see CHANGELOG, "WFDateFormat vs WFDateFormatString"). Always set `WFDate` (not `WFInput`) with the source Date. See `DATE_TIME.md` for UNIX timestamp, ISO 8601, RFC 2822, and Unicode TR35 guidance.
...
- If `shortcuts sign` says `The file couldn't be opened because it isn't in the correct format` while the XML passes `validate_shortcut.py` and `plutil -lint`, retry after `plutil -convert binary1` on the final `.shortcut` copy. The bundled signer wrapper performs this retry automatically; if both XML and binary signing fail, then suspect Codex `workspace-write` sandbox restrictions before treating the plist as malformed.
