...
For macOS 27 schema grounding, the package includes `data/macos27-shortpy-grounding.json`, `data/toolkit-v78-first-party-parameter-keys.json`, `data/toolkit-v78-first-party-enum-cases.json`, `data/toolkit-v78-trigger-parameter-keys.json`, `data/macos27-workflow-trigger-samples.json`, and `scripts/lookup_action_grounding.py`. This is reviewed static metadata derived from Apple's local ToolKit/framework surfaces and exported shortcut XML; normal validation does not read live Shortcuts databases or call private frameworks.

If `scripts/sign_shortcut.sh` fails with `Error: The file couldn't be opened because it isn't in the correct format.` while `validate_shortcut.py` and `plutil -lint` pass, the wrapper first retries after binary plist conversion. If XML and binary signing both fail, retry outside Codex `workspace-write` sandbox restrictions or with Codex filesystem sandboxing set to full access.
